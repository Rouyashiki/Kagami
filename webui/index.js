...
const TABS = [
  ...
  { id: "kasumi", key: "nav.kasumi", icon: "kasumi" },
...
const storedLanguage = localStorage.getItem("kagami_static_language");
...
const state = {
  ...
  theme: localStorage.getItem("kagami_static_theme") || "system",
  ...
  error: "",
  lastUpdated: "",
  warningAccepted: getRuntimeMode() === "mock" || localStorage.getItem("kagami_experimental_ack") === "true",
  warningCountdown: 5,
};
...
function getModeLabel(mode) {
  switch (mode) {
    ...
    case "kasumi":
      return tr("modules.modeKagamifs", "Kasumi");
    case "overlay":
...
function setLanguage(language) {
  ...
  localStorage.setItem("kagami_static_language", language);
  applyLanguage();
  localizeBootShell();
  renderApp();
}
...
function cycleTheme() {
  ...
  localStorage.setItem("kagami_static_theme", state.theme);
  applyTheme();
  renderApp();
}
...
function filteredModules() {
  return state.modules.filter((module) => {
    ...
    const matchesFilter =
      ...
      (state.moduleFilter === "kasumi-active" && (state.systemInfo.kasumiModules || []).includes(module.id));
    ...
    <div class="page-grid">
      <section class="card home-console">
        <div class="home-console-head">
          <div class="brand">
            <div class="brand-icon"><img src="./icon.svg" alt="Kagami" width="24" height="24"></div>
      ...
      <section class="metric-grid">
        ...
        <article class="metric-card">
          <div class="metric-label">${escapeHtml(tr("staticUi.overview.kasumiActive", "Kasumi Active"))}</div>
          <div class="metric-value">${(state.systemInfo.kasumiModules || []).length}</div>
          <div class="metric-foot">${escapeHtml(tr("nav.kasumi", "Kasumi"))}</div>
        </article>
        <article class="metric-card">
      ...
      ${
        state.systemInfo.kasumiMismatch
          ? `<section class="banner" data-tone="danger"><strong>${escapeHtml(tr("status.kasumiMismatch", "Kasumi Protocol Mismatch"))}</strong><div>${escapeHtml(
        ...
        <section class="card">
          ...
          <div class="info-list">
            ...
            <div class="info-item"><div class="label">${escapeHtml(tr("staticUi.common.protocol", "Protocol"))}</div><div class="value">${escapeHtml(
              state.systemInfo.kasumiMismatch ? tr("status.kasumiMismatch", "Mismatch") : tr("staticUi.common.ok", "OK")
            )}</div></div>
            <div class="info-item"><div class="label">Kasumi</div><div class="value">${escapeHtml(
              state.systemInfo.kasumiAvailable ? tr("staticUi.common.available", "Available") : tr("staticUi.common.unavailable", "Unavailable")
            )}</div></div>
            <div class="info-item"><div class="label">LKM</div><div class="value">${escapeHtml(
              state.lkmStatus.loaded ? tr("kasumi.lkm.loaded", "Loaded") : tr("kasumi.lkm.notLoaded", "Not Loaded")
            )}</div></div>
          </div>
        </section>
      </div>
        ...
        <section class="card">
          ...
          <div class="field-grid">
            ...
            <div class="field">
              ...
              <input id="cfg-tempdir" name="tempdir" value="${escapeHtml(config.tempdir)}" placeholder="/data/adb/kagami/img_mnt">
            </div>
            <div class="field">
          ...
          <div class="search-box">
            <select data-role="module-filter">
              ${[
                ...
                { value: "kasumi", label: tr("modules.filterKagamifs", "Kasumi") },
                { value: "overlay", label: tr("modules.filterOverlay", "OverlayFS") },
                { value: "magic", label: tr("modules.filterMagic", "Magic Mount") },
                { value: "kasumi-active", label: tr("staticUi.modules.kasumiActive", "Kasumi Active") },
              ]
    ...
    <div class="page-grid">
      <div class="grid-2">
        <section class="card">
          <div class="section-head">
            <div>
              <h3>${escapeHtml(tr("kasumi.lkm.title", "Kasumi Kernel Module (LKM)"))}</h3>
            </div>
            <div class="badge-row">
              ${
                state.lkmStatus.loaded
                  ? `<span class="badge" data-tone="success">${escapeHtml(tr("kasumi.lkm.loaded", "Loaded"))}</span>`
                  : `<span class="badge">${escapeHtml(tr("kasumi.lkm.notLoaded", "Not Loaded"))}</span>`
              }
              ${
                state.lkmStatus.autoload
                  ? `<span class="badge" data-tone="primary">${escapeHtml(tr("kasumi.lkm.autoload", "Autoload"))}</span>`
          ...
          <div class="field-inline" style="margin-top:14px;">
            <div class="field">
              <label for="kmi-input">${escapeHtml(tr("kasumi.lkm.kmiOverride", "KMI Override"))}</label>
              <input id="kmi-input" value="${escapeHtml(state.lkmStatus.kmi_override || "")}" placeholder="android14-6.1">
            </div>
            <button class="button" data-variant="secondary" data-action="set-kmi">${escapeHtml(tr("kasumi.lkm.setKmi", "Set"))}</button>
      ...
      <div class="grid-2">
        <section class="card">
          <div class="section-head">
            <div>
              <h3>${escapeHtml(tr("staticUi.kasumi.featureFlags", "Feature Flags"))}</h3>
            </div>
          </div>
        ...
        <section class="card">
          <div class="section-head">
            <div>
              <h3>${escapeHtml(tr("kasumi.hooks.title", "Kernel Hooks"))}</h3>
            </div>
          </div>
          <div class="mono-block">${escapeHtml(state.systemInfo.hooks || tr("staticUi.kasumi.noHookData", "No hook data"))}</div>
        </section>
      </div>
      ...
      <section class="card">
        <div class="section-head">
          ...
          <div class="badge-row">
            <span class="badge" data-tone="primary">${escapeHtml(tr("staticUi.kasumi.userRulesCount", "{count} user rules", { count: state.userHideRules.length }))}</span>
            <span class="badge">${escapeHtml(tr("staticUi.kasumi.totalRulesCount", "{count} total rules", { count: state.allRules.length }))}</span>
          </div>
        </div>
        <div class="field-inline">
          <div class="field">
      ...
      <section class="card">
        <div class="section-head">
          <div>
            <h3>${escapeHtml(tr("staticUi.kasumi.allEffectiveRules", "All Effective Rules"))}</h3>
        ...
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>${escapeHtml(tr("staticUi.kasumi.type", "Type"))}</th>
                <th>${escapeHtml(tr("modules.path", "Path"))}</th>
                <th>${escapeHtml(tr("staticUi.kasumi.target", "Target"))}</th>
                <th>${escapeHtml(tr("staticUi.kasumi.source", "Source"))}</th>
                <th>${escapeHtml(tr("staticUi.kasumi.origin", "Origin"))}</th>
              </tr>
            </thead>
            <tbody>
              ${state.allRules
                .map(
                  (rule) => `
                    <tr>
                      <td data-label="${escapeHtml(tr("staticUi.kasumi.type", "Type"))}">${escapeHtml(rule.type || tr("staticUi.common.unknown", "Unknown"))}</td>
                      <td class="mono" data-label="${escapeHtml(tr("modules.path", "Path"))}">${escapeHtml(rule.path || "-")}</td>
                      <td class="mono" data-label="${escapeHtml(tr("staticUi.kasumi.target", "Target"))}">${escapeHtml(rule.target || "-")}</td>
                      <td class="mono" data-label="${escapeHtml(tr("staticUi.kasumi.source", "Source"))}">${escapeHtml(rule.source || "-")}</td>
                      <td data-label="${escapeHtml(tr("staticUi.kasumi.origin", "Origin"))}">${escapeHtml(
                        rule.isUserDefined ? tr("staticUi.kasumi.userDefined", "user") : tr("staticUi.kasumi.moduleKernel", "module/kernel")
                      )}</td>
                    </tr>
                  `
                )
      ...
      <div class="app-frame">
        ...
        <main class="page-shell">
          <div class="page-carousel is-pending" id="page-carousel">
            ...
            <section class="page-pane" data-page="kasumi"><div class="page-pane-scroll">${renderKasumiPage()}</div></section>
  ...
  return `
    <div class="overlay">
      <div class="modal">
        ...
        <h2>${escapeHtml(tr("staticUi.warning.headline", "Kasumi is still experimental"))}</h2>
  ...
  containers.forEach((container) => {
    ...
    const rules = [...container.querySelectorAll(".rule-row")]
      .map((row) => ({
        ...
        mode: row.querySelector("[data-rule-mode]")?.value || "kasumi",
      }))
      .filter((rule) => rule.path);
...
async function handleSetKmi(clear = false) {
  try {
    ...
    if (clear) {
      ...
      showToast(tr("kasumi.lkm.kmiClearSuccess", "KMI override cleared"));
    } else {
      await api.lkmSetKmi(value);
      showToast(tr("kasumi.lkm.kmiSetSuccess", "KMI override set"));
    }
    state.lkmStatus = await api.getLkmStatus();
    queueRenderMotion("page");
    renderApp();
    ...
    showToast(
      error.message ||
        (clear ? tr("kasumi.lkm.kmiClearFailed", "Failed to clear KMI override") : tr("kasumi.lkm.kmiSetFailed", "Failed to set KMI override")),
      "danger"
    );
  }
}
...
function handleAddModuleRule(moduleId) {
  ...
  target.rules = [...(target.rules || []), { path: "", mode: "kasumi" }];
  queueRenderMotion("module", `.module-card[data-module-id="${moduleId}"]`);
  renderApp();
}
...
function acceptWarning() {
  ...
  localStorage.setItem("kagami_experimental_ack", "true");
  renderApp();
}
